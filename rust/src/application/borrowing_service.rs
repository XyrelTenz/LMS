...
pub fn borrow_book(user_id: String, user_name: String, book_id: String) -> Result<(), String> {
    ...
    let due_date_dt = borrow_date_dt + chrono::Duration::hours(24);

    conn.execute(
        "INSERT INTO borrowings (id, book_id, user_id, borrower_name, borrow_date, due_date, is_returned, status, has_reminder) VALUES (?1, ?2, ?3, ?4, ?5, ?6, 0, 'Pending', 0)",
...
/// Finalizes a return by setting the return date and making the book available.
pub fn return_book(borrowing_id: String) -> Result<(), String> {
    ...
    conn.execute(
        "UPDATE borrowings SET is_returned = 1, return_date = ?1 WHERE id = ?2",
        params![Utc::now().to_rfc3339(), borrowing_id],
    )
    .map_err(|e| e.to_string())?;
...
pub fn get_user_borrowings(user_id: String) -> Result<Vec<Borrowing>, String> {
    let conn = sqlite::init_db().map_err(|e| e.to_string())?;
    let mut stmt = conn.prepare("SELECT id, book_id, user_id, borrower_name, borrow_date, return_date, is_returned, status, due_date, has_reminder FROM borrowings WHERE user_id = ?1")
        .map_err(|e| e.to_string())?;
...
pub fn get_pending_borrowings() -> Result<Vec<Borrowing>, String> {
    let conn = sqlite::init_db().map_err(|e| e.to_string())?;
    let mut stmt = conn.prepare("SELECT id, book_id, user_id, borrower_name, borrow_date, return_date, is_returned, status, due_date, has_reminder FROM borrowings WHERE status = 'Pending'")
        .map_err(|e| e.to_string())?;
...
pub fn get_all_borrowings() -> Result<Vec<Borrowing>, String> {
    let conn = sqlite::init_db().map_err(|e| e.to_string())?;
    let mut stmt = conn.prepare("SELECT id, book_id, user_id, borrower_name, borrow_date, return_date, is_returned, status, due_date, has_reminder FROM borrowings")
        .map_err(|e| e.to_string())?;
