use crate::domain::report::{LibraryReport, ActiveBorrower};
...
pub fn generate_report() -> Result<LibraryReport, String> {
    ...
    let total_books: i32 = conn.query_row("SELECT COUNT(*) FROM books", [], |row| row.get(0)).unwrap_or(0);
    let available_books: i32 = conn.query_row("SELECT COUNT(*) FROM books WHERE copies > 1", [], |row| row.get(0)).unwrap_or(0);
    let borrowed_books: i32 = conn.query_row("SELECT COUNT(*) FROM borrowings WHERE is_returned = 0 AND status = 'Approved'", [], |row| row.get(0)).unwrap_or(0);
    let total_users: i32 = conn.query_row("SELECT COUNT(*) FROM users", [], |row| row.get(0)).unwrap_or(0);

    let mut stmt = conn.prepare("SELECT genre, COUNT(*) FROM books GROUP BY genre").map_err(|e| e.to_string())?;
    ...
    let mut stmt = conn.prepare(
        "SELECT DISTINCT b.id, b.title, b.author, b.publication_year, b.isbn, b.genre, b.copies, b.is_available, b.image_url 
         FROM books b JOIN borrowings br ON b.id = br.book_id 
         WHERE br.is_returned = 0 AND br.status = 'Approved'"
    ).map_err(|e| e.to_string())?;

    let borrowed_books_list = stmt.query_map([], |row| {
        Ok(Book {
            id: row.get(0)?,
            title: row.get(1)?,
            author: row.get(2)?,
    ...
    let active_borrowers = stmt.query_map([], |row| {
        Ok(ActiveBorrower {
            user_id: row.get(0)?,
            borrower_name: row.get(1)?,
            book_title: row.get(2)?,
            borrow_date: row.get(3)?,
            due_date: row.get::<_, Option<String>>(4)?.unwrap_or_else(|| "N/A".to_string()),
        })
    }).map_err(|e| e.to_string())?
    .collect::<Result<Vec<_>, _>>().map_err(|e| e.to_string())?;
